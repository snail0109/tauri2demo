...
function Install-WingetTool {
  ...
  if (-not $installed) {
    Write-Host ""
    Write-Host "  方式二：通过 Microsoft Store 安装" -ForegroundColor Yellow
...
function Write-Usage {
  ...
  Write-Host "  .\install_base_tools_bywin.ps1 -Tools <工具1,工具2,...>  安装指定工具"
  Write-Host "  .\install_base_tools_bywin.ps1 -Tools all               安装所有工具"
  Write-Host "  .\install_base_tools_bywin.ps1 -y -Tools all            静默安装所有工具"
  Write-Host ""
  ...
  Write-Host "  .\install_base_tools_bywin.ps1 -Tools winget"
  Write-Host "  .\install_base_tools_bywin.ps1 -Tools winget,terminal"
  Write-Host "  .\install_base_tools_bywin.ps1 -Tools all"
  Write-Host ""
}
...
# 校验参数
$unknown = $Tools | Where-Object { $_ -notin $validIds }
if ($unknown) {
...
$needsWinget = $Tools | Where-Object { $_ -ne 'winget' }
if ($needsWinget -and -not (Get-ExePath 'winget.exe') -and 'winget' -notin $Tools) {
