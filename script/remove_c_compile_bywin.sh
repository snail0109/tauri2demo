...
# 功能：
#   按用户选择卸载以下任一组合：
#     1) Rust(gnu) + MinGW gcc + rustup + MSYS2
#     2) Rust(msvc) + MSVC + rustup + MSYS2
#     3) 全部卸载（rustup + MSYS2 + MSVC，最危险，需多次确认）
#
...
remove_mingw_packages() {
  ...
  if ! command -v pacman &>/dev/null; then
    if [[ ! -f "/c/msys64/usr/bin/bash.exe" ]]; then
      warn "未找到 /c/msys64/usr/bin/bash.exe，无法调用 pacman"
      return 1
    fi
  fi
  ...
  if confirm_remove "卸载 ${pkgs}"; then
    if /c/msys64/usr/bin/bash.exe -lc "pacman -R --noconfirm ${pkgs}" 2>&1; then
      ok "已卸载 ${pkgs}"
    else
...
# 解决：Windows 用户 PATH 已更新但当前 shell 未刷新时，bare `command -v rustup`
# 会误判 rustup 未安装。
probe_cargo_bin() {
